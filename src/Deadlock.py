...
def deadlock_detection(
    ...
    # n_trans và n_places lần lượt là số lượng transition và places
    n_trans, n_places = I.shape
    ...
    # Hàm kiểm tra transition t_index có bắn được từ marking M hay không
    def is_fireable(M: np.ndarray, t_index: int) -> bool:
        # pre và post là các hàng của ma trận I và O tương ứng với mỗi hàng là một transition
        pre = I[t_index, :]
        post = O[t_index, :]
    ...
    # Danh sách tất cả deadlock markings
    ...
    for point in bdd.satisfy_all():
        ...
        # Nếu không có transition nào bắn được -> đây là deadlock marking
        if not any_fireable:
            deadlock_marking = M.tolist()
            deadlocks.append(deadlock_marking)
            # Nếu muốn in ra từng deadlock:
            print("Deadlock marking:", deadlock_marking)
